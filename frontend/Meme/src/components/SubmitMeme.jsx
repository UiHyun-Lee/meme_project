...
const SubmitMeme = () => {
  ...
  const handleMemeCreate = async () => {
    try {
      ...
      const container = document.getElementById("imgContainer");
      if (!container) {
        alert("Meme container not found!");
        return;
      }
      ...
      form.append(
        "image_file",
        new File([blob], "meme.jpg", { type: "image/jpeg" })
      );
      form.append("caption", "User created meme");
      form.append("created_by", "human");
      form.append("template_id", selectedTemplate?.id || "");
      ...
      // 🔥 백엔드에 문자열 topic 전송 (Meme.topic 필드와 매칭)
      // MemeMinimalSerializer / MemeSerializer 둘 다 topic 필드 그대로 사용 중
      if (currentTopic?.name) {
        form.append("topic", currentTopic.name);
      } else {
        // 토픽이 없어도 업로드는 되게 하고 싶으면 이 부분은 그냥 생략해도 됨
        form.append("topic", "");
      }
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Typewriter
            text="Create a Meme. Challenge the Machines."
            speed={50}
        ...
        >
          <Typewriter
            text="Use our photo editor to create your meme and join the competition!"
            speed={50}
      ...
      {uploading && (
        ...
        >
          ...
          <p style={{ color: "#666" }}>
            Your meme is being processed and uploaded.
          </p>
        </div>
      )}
